...
def handle_get_each_character(people_id):
    people = Characters.query.filter_by(id=people_id).first()
    return jsonify(people), 200


@app.route('/favorite/people/<int:people_id>', methods=['POST'])
def handle_favorite_character(people_id):
    body = request.get_json()
    user_id = body['id']
    newfav = Favorites(id=user_id, character_id=people_id, planet_id=None)
...
def handle_delete_favorite_character(people_id):
    body = request.get_json()
    user_id = body['id']
    fav = Favorites.query.filter_by(id=user_id, character_id=people_id).first()
...
def handle_favorite_planet(planet_id):
    body = request.get_json()
    user_id = body['id']
    newfav = Favorites(id=user_id, character_id=planet_id, people_id=None)
    db.session.add(newfav)
    db.session.commit()
    return jsonify('Favorite has been created'), 200
...
def handle_delete_favorite_planet(planet_id):
    body = request.get_json()
    user_id = body['id']
    fav = Favorites.query.filter_by(id=user_id, character_id=planet_id).first()
    db.session.delete(fav)
    db.session.commit()
    return jsonify('Favorite has been created'), 200
